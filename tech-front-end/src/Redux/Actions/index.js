...
export const getAssignmentById = id => async dispatch => {
  try {
    let response = await axios.get(
      `http://localhost:5000/api/assignments/id/${id}`
    );
...
export const getAllEmployeeAssignments = id => async dispatch => {
  try {
    let response = await axios.get(
      `http://localhost:5000/api/${id}/assignments/all$`
    );
...
export const getEmployeesInProject = () => async dispatch => {
  try {
    let id = 1;
    let response = await axios.get(
      `http://localhost:5000/api/projects/id/${id}/employees`
    );
    console.log("THIS IS THE RESPONSE: ", response);
    dispatch({
      type: GET_EMPLOYEES_IN_PROJECT,
...
export const getAssignmentsInProject = () => async dispatch => {
  try {
    let id = 1;
    let response = await axios.get(
      `http://localhost:5000/api/projects/id/${id}/assignments`
    );
    console.log("assignment response: ", response);
    dispatch({
      type: GET_ASSIGNMENTS_IN_PROJECT,
...
export const getAllProjectRoles = () => async dispatch => {
  try {
    let response = await axios.get(
      "http://localhost:5000/api/projects/oles/all"
    );
...
export const updateProjectRole = (project_role, id) => async dispatch => {
  try {
    let id = 9;
    let response = await axios.put(
      `http://localhost:500/api/projects/roles/${id}`,
      project_role
    );
    dispatch({
      type: UPDATE_PROJECT_ROLES,
